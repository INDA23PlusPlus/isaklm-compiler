...
bool match_keyword(std::string source_code, int offset, Token& token)
{
	...
	{
		"int", "vec", "if", "else", "while", "def", "return", "print"
	};
...
std::string get_identifier(std::string source_code, int offset)
{
	...
	for(int i = 0; ; i++)
	{
		...
		if (string_index >= source_code.size() || !(isalpha(character) || isdigit(character) || character == '_'))
		{
			break;
		}
...
std::vector<Token> lexer(std::string file_name)
{
	...
	while(index < source_code.size())
	{
		...
		else
		{
			...
			if (!match_constant(source_code, index, token))
			{
				if (!match_symbol(source_code, index, token))
				{
					if (!match_keyword(source_code, index, token))
					{
